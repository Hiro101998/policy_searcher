    ...
    methods: {
        ...
        search_universities() {
            ...
            if (this.keyField.length > 0) {
                ...
                let field_1_Filter = searchProcess.filter(x =>
                    searchField.includes(x.field_id_1)
                );
                let field_2_Filter = searchProcess.filter(x =>
                    searchField.includes(x.field_id_2)
                );
                let field_3_Filter = searchProcess.filter(x =>
                    searchField.includes(x.field_id_3)
                );
                let field_4_Filter = searchProcess.filter(x =>
                    searchField.includes(x.field_id_4)
                );
                //concatで結合する
                let join = field_1_Filter.concat(
                    field_2_Filter,
                    field_3_Filter,
                    field_4_Filter
                );
            ...
            if (this.keyRegion.length > 0) {
                ...
                let regionFilter = searchProcess.filter(x =>
                    searchRegion.includes(x.region_id)
                );
            ...
            if (this.keyPrefecture.length > 0) {
                ...
                let prefectureFilter = searchProcess.filter(x =>
                    searchPrefecture.includes(x.prefecture_id)
                );
            ...
            if (this.keyOperator.length > 0) {
                ...
                let operatorFilter = searchProcess.filter(x =>
                    searchOperator.includes(x.operator_id)
                );
